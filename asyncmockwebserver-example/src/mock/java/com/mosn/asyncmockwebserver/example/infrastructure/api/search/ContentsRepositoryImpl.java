...
public class ContentsRepositoryImpl implements ContentsRepository {
  ...
  @Inject RestAdapter.Builder restAdapterBuilder;
  ...
  public void search(final String keyword, @NonNull final NetworkCallback<Contents> callback) {
    ...
    restAdapterBuilder.setEndpoint(AsyncMockWebServer.getEndPoint());
    restAdapterBuilder
        .build()
        .create(SearchApi.class)
        .search(
            keyword,
            new Callback<SearchApiDto>() {
  ...
  private List<Content> convertContentList(
      @NonNull List<Content> contents, @NonNull List<SearchApiDto.Result> results) {
