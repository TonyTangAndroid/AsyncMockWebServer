...
public final class AsyncMockWebServer implements Handler.Callback {
  ...
  @NonNull
  private AsyncMockWebServerListener listener =
      new AsyncMockWebServerListener() {
        @Override
  ...
  public static void start(
      @Nullable MockWebServerConfig config, @Nullable AsyncMockWebServerListener listener) {
  ...
  private void sendStartCommand(@Nullable MockWebServerConfig config) {
    if (!existThread()) return;
    Message message =
        MessageUtils.obtain(threadHandler, config, AsyncMockWebServerThread.COMMAND_START);
  ...
  private void sendAddMockCommand(@NonNull Mock mock) {
    if (!existThread()) return;
    Message msg =
        MessageUtils.obtain(threadHandler, mock, AsyncMockWebServerThread.COMMAND_ADD_MOCK);
  ...
  private void sendDeleteMockCommand(@NonNull String key) {
    if (!existThread()) return;
    Message msg =
        MessageUtils.obtain(threadHandler, key, AsyncMockWebServerThread.COMMAND_DELETE_MOCK);
    threadHandler.sendMessage(msg);
  }
